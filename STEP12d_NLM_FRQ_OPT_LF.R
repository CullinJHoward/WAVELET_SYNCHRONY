...
plot_dir <- "LF_PHASE_NLM_PLOTS"
if (!dir.exists(plot_dir)) dir.create(plot_dir)

csv_dir <- "LO_FRQ_PHS_LONG"
...
for (csv_file in csv_files) {
  ...
  if (!is.null(best_model)) {
    ...
    pdf_file <- file.path(plot_dir, paste0("LO_PHS_NLM_PLOT_", id, ".pdf"))
...
write.csv(RED_SUMMARY, "..//LF_NLM_PHS_SUMMARY.csv", row.names = FALSE)
