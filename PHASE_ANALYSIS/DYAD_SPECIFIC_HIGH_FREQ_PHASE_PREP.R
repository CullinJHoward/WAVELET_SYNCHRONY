...
################################################################################
############## TRIM DATA TO HIGH FREQUENCY BAND
...
for (df_name in coherence_dfs) {
  ...
  reduced_df <- df[df$Period >= 2.5 & df$Period <= 6.6, ]
...
################################################################################
######################  TIME-VARYING HIGH-FREQUENCY PHASE CHANGES 
...
opt_dfs <- ls(pattern = "^HF_", envir = .GlobalEnv)
if (length(opt_dfs) == 0) {
  stop("No data frames with prefix 'HF_' found in the environment.")
}
...
ds_opt_dfs <- ls(pattern = "^DS_HF_", envir = .GlobalEnv)
if (length(ds_opt_dfs) == 0) {
  stop("No data frames with prefix 'DS_HF_' found in the environment.")
}
...
median(HF_0002$Period)

################################################################################
################## REFINE THE PERIOD TO BE TIGHTER 

# Define the target Period values
target_periods <- c(3.60500185044332, 3.73213196614723, 3.86374531569938, 4, 
                    4.14105969536551, 4.28709385014517, 4.43827788827138, 4.59479341998814)

# Get all data frames with prefix "DS_HF_"
ds_hf_dfs <- ls(pattern = "^DS_HF_")

# Loop through each data frame
for (df_name in ds_hf_dfs) {
  # Get the data frame
  df <- get(df_name)
  ...
  # Filter rows where Period is in target_periods (using %in% for exact matching)
  filtered_df <- df[df$Period %in% target_periods, ]
...
# Check how many filtered data frames were created
length(ls(pattern = "^T_DS_HF_"))
...
# Identify all data frames with the DS_HF_ prefix
ds_opt_dfs <- ls(pattern = "^T_DS_HF_", envir = .GlobalEnv)
if (length(ds_opt_dfs) == 0) {
  stop("No data frames with prefix 'T_DS_HF_' found in the environment.")
}

# Loop through each T_DS_HF_ data frame and convert to long format
for (df_name in ds_opt_dfs) {
  # Retrieve the data frame
  df <- get(df_name, envir = .GlobalEnv)
  
  # Extract ID from the suffix (e.g., "1234" from "DS_OPT_1234")
  id <- sub("^T_DS_HF_", "", df_name)
...
folder_name <- "LONG_HIGH_FREQ_PHASE_SERIES"
...
long_ds_hf_dfs <- ls(pattern = "^LONG_T_DS_HF_", envir = .GlobalEnv)
if (length(long_ds_hf_dfs) == 0) {
  stop("No data frames with prefix 'LONG_T_DS_HF_' found in the environment.")
}

# Loop through each LONG_T_DS_HF_ data frame and save as CSV
for (df_name in long_ds_hf_dfs) {
  # Retrieve the data frame
  df <- get(df_name, envir = .GlobalEnv)
  
  # Extract the 4-digit ID from the data frame name
  id <- sub(".*LONG_T_DS_HF_(\\d{4}).*", "\\1", df_name)
  
  # Define the CSV file path using the extracted ID
  csv_file <- file.path(folder_name, paste0("LONG_T_DS_HF_", id, ".csv"))
