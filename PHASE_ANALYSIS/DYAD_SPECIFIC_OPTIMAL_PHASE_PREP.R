...
################################################################################
######################  TIME-VARYING OPTIMIZED PHASE CHANGES 
...
# Identify all data frames with the OPT_ prefix and apply downsampling for phase
...
################################################################################
################## LETS REDUCE TO A TIGHTER OPTIMIZED BAND 
...
# Ensure POWER_SUMMARY_DF exists
if (!exists("POWER_SUMMARY_DF", envir = .GlobalEnv)) {
  stop("POWER_SUMMARY_DF not found in the global environment.")
}

# Loop through each DS_OPT_ data frame and reduce to 5 rows around OPT_PERIOD
for (df_name in ds_opt_dfs) {
  # Retrieve the data frame
  df <- get(df_name, envir = .GlobalEnv)
  
  # Extract ID from the data frame name (e.g., "1234" from "DS_OPT_1234")
  id <- sub("^DS_OPT_", "", df_name)
  
  # Find the OPT_PERIOD for this ID in POWER_SUMMARY_DF
  opt_period_row <- POWER_SUMMARY_DF[POWER_SUMMARY_DF$ID == id, ]
  if (nrow(opt_period_row) == 0) {
    warning(paste("No matching ID", id, "found in POWER_SUMMARY_DF. Skipping."))
    next
  }
  if (nrow(opt_period_row) > 1) {
    warning(paste("Multiple rows for ID", id, "found in POWER_SUMMARY_DF. Using first."))
    opt_period_row <- opt_period_row[1, ]
  }
  opt_period <- opt_period_row$OPT_PERIOD
  
  # Check if OPT_PERIOD is valid
  if (is.na(opt_period)) {
    warning(paste("OPT_PERIOD is NA for ID", id, ". Skipping."))
    next
  }
  
  # Sort the data frame by Period to ensure order
  df <- df[order(df$Period), ]
  
  # Find the index of the row where Period matches OPT_PERIOD (or is closest)
  period_diff <- abs(df$Period - opt_period)
  opt_index <- which.min(period_diff)
  
  # Define the range of rows: 2 above, 1 at OPT_PERIOD, 2 below (5 total)
  start_index <- max(1, opt_index - 2)  # Ensure not below 1
  end_index <- min(nrow(df), opt_index + 2)  # Ensure not above nrow
  
  # Adjust if fewer than 5 rows are available
  if (end_index - start_index + 1 < 5) {
    # Center around opt_index as much as possible
    rows_needed <- 5 - (end_index - start_index + 1)
    if (start_index > 1) {
      start_index <- max(1, start_index - rows_needed)
    }
    if (end_index < nrow(df) && end_index - start_index + 1 < 5) {
      end_index <- min(nrow(df), end_index + (5 - (end_index - start_index + 1)))
    }
  }
  
  # Subset the data frame to the 5 rows (or fewer if at edges)
  df_reduced <- df[start_index:end_index, , drop = FALSE]
  
  # Overwrite the original data frame with the reduced version
  assign(df_name, df_reduced, envir = .GlobalEnv)
  
  # Print progress
  cat(sprintf("Reduced %s to %d rows around OPT_PERIOD = %.2f (Period range: %.2f to %.2f)\n", 
              df_name, nrow(df_reduced), opt_period, min(df_reduced$Period), max(df_reduced$Period)))
}
...
for (df_name in ds_opt_dfs) {
  ...
  # Check if OPT_PERIOD is valid
  if (is.na(opt_period)) {
    warning(paste("OPT_PERIOD is NA for ID", id, "in POWER_SUMMARY_DF. Skipping."))
    next
  }
  ...
  # Create PER_CEN: Period centered at OPT_PERIOD
  df_long$PER_CEN <- df_long$Period - opt_period
  ...
  # Reorder and select final columns: ID, Period, PER_CEN, TIME, PHASE
  df_long <- df_long[, c("ID", "Period", "PER_CEN", "TIME", "PHASE")]
  ...
  # Print progress
  cat(sprintf("Converted %s to %s: %d rows, %d columns, OPT_PERIOD = %.2f\n", 
              df_name, new_df_name, nrow(df_long), ncol(df_long), opt_period))
}
...
folder_name <- "LONG_OPTIMIZED_PHASE_SERIES"
...
long_ds_opt_dfs <- ls(pattern = "^LONG_DS_OPT_", envir = .GlobalEnv)
if (length(long_ds_opt_dfs) == 0) {
  stop("No data frames with prefix 'LONG_DS_OPT_' found in the environment.")
}

# Loop through each LONG_DS_OPT_ data frame and save as CSV
for (df_name in long_ds_opt_dfs) {
  # Retrieve the data frame
  df <- get(df_name, envir = .GlobalEnv)
  
  # Extract the 4-digit ID from the data frame name
  id <- sub(".*LONG_DS_OPT_(\\d{4}).*", "\\1", df_name)
  
  # Define the CSV file path using the extracted ID
  csv_file <- file.path(folder_name, paste0("LONG_DS_OPT_", id, ".csv"))
