...
plot_dir <- "HF_PHASE_NLME_PLOTS"
if (!dir.exists(plot_dir)) dir.create(plot_dir)

# Define the folder containing CSV files
csv_dir <- "F:/SRCD_25_ANALYSIS/WAVELET/LONG_HIGH_FREQ_PHASE_SERIES"
...
for (csv_file in csv_files) {
  ...
  if (!all(c("PHASE", "TIME", "Period") %in% names(data))) {
    warning(sprintf("CSV %s missing PHASE, TIME, or Period columns. Skipping.", basename(csv_file)))
    next
  }
  ...
  # Generate and save plot with Period rank coloring
  data$pred <- predict(best_model)
  # Rank unique Period values
  unique_periods <- sort(unique(data$Period))
  n_periods <- length(unique_periods)
  if (n_periods < 8) {
    warning(sprintf("ID %s has fewer than 8 unique Period values (%d). Using dynamic ranking.", id, n_periods))
    n_top <- n_bottom <- floor(n_periods / 3)
    n_middle <- n_periods - n_top - n_bottom
  } else {
    n_top <- n_bottom <- 3
    n_middle <- 2
  }
  # Assign categories
  bottom_periods <- unique_periods[1:n_bottom]
  middle_periods <- unique_periods[(n_bottom + 1):(n_bottom + n_middle)]
  top_periods <- unique_periods[(n_bottom + n_middle + 1):n_periods]
  data$Period_cat <- ifelse(data$Period %in% bottom_periods, "Bottom Periods",
                            ifelse(data$Period %in% middle_periods, "Middle Periods",
                                   "Top Periods"))
  data$Period_cat <- factor(data$Period_cat, 
                            levels = c("Bottom Periods", "Middle Periods", "Top Periods"))
  
  p <- ggplot(data, aes(x = TIME, y = PHASE)) +
    geom_point(aes(color = Period_cat), alpha = 0.5, size = 1.5) +
    geom_line(aes(y = pred), color = "black", size = 1) +
    scale_color_manual(values = c("Bottom Periods" = "blue", 
                                  "Middle Periods" = "gray50", 
                                  "Top Periods" = "red")) +
    labs(title = paste("NLS Fit for ID", id), 
         x = "TIME", 
         y = "PHASE", 
         color = "Period Category") +
    theme_minimal() +
    theme(legend.position = "bottom")
  
  pdf_file <- file.path(plot_dir, paste0("HF_PHS_NLME_PLOT_", id, ".pdf"))
...
write.csv(summary_df, "HF_NLME_PHASE_SUMMARY.csv", row.names = FALSE)
cat("Summary saved to HF_NLME_PHASE_SUMMARY.csv\n")
