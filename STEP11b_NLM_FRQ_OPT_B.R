...
plot_dir <- "OPT_COH_Vb_NLM_PLOTS"
if (!dir.exists(plot_dir)) dir.create(plot_dir)

csv_dir <- "FRQ_OPT_VERSION_B_LONG"
...
for (csv_file in csv_files) {
  ...
  if (!is.null(best_model)) {
    ...
    pdf_file <- file.path(plot_dir, paste0("OPT_COH_Vb_NLM_PLOT_", id, ".pdf"))
...
RED_SUMMARY <- summary_df %>%
  ...
  rename(FOb_MU = mu_est,
         FOb_A = a_est, 
         FOb_B = b_est,
         FOb_C = c_est,
         FOb_R2 = R2)

write.csv(RED_SUMMARY, "..//FRQ_OPT_Vb_NLM_COH_SUMMARY.csv", row.names = FALSE)
