...
################################################################################
##################### LOAD IN THE PHASE DFs


# LOCATE THE POWER MEAN DATA FROM WITHIN THE COI
folder <- file.path("ANGLE_FILTERED\\")
...
################################################################################
############## TRIM DATA TO OPTIMIZED PHASE BANDS PREVIOUSLY IDENTIFIED
...
################################################################################
###################### EXTRACT TIME-VARYING OPTIMIZED PHASE VALUES 

##################  DOWNSAMPLE TO 1 SECOND INTERVALS 

# Helper: circular mean for a vector, returns NA if all NA
circular_mean_na <- function(x) {
  if (all(is.na(x))) return(NA_real_)
  atan2(sum(sin(x), na.rm = TRUE), sum(cos(x), na.rm = TRUE))
}

reduce_columns_circular <- function(df, df_name) {
  # Ensure all columns except Hz are numeric
  df[, -1] <- lapply(df[, -1, drop = FALSE], as.numeric)  # Skip column 1 (Hz)
  
  time_series_cols <- ncol(df) - 1  # Exclude Hz column
  ...
  # Only use the first 20*num_groups columns after Hz
  used_cols <- (2):(1 + 20 * num_groups)
  df_trimmed <- df[, c(1, used_cols), drop = FALSE]
  ...
  df_reduced <- as.data.frame(do.call(cbind, lapply(seq_len(num_groups), function(i) {
    start_col <- (i - 1) * 20 + 2  # Start at column 2 (after Hz)
    end_col <- start_col + 19      # Fixed 20-column block for 1 Hz
    apply(df_trimmed[, start_col:end_col, drop = FALSE], 1, circular_mean_na)
  })))
  ...
  df_reduced <- cbind(Hz = df_trimmed$Hz, df_reduced)
...
# Identify all data frames with the OPT_ prefix
opt_dfs <- ls(pattern = "^OPT_", envir = .GlobalEnv)


# Apply the circular downsampling function to each
for (df_name in opt_dfs) {
  df <- get(df_name, envir = .GlobalEnv)  # Retrieve data frame
  reduce_columns_circular(df, df_name)    # Apply downsampling function
}
...
for (df_name in ds_opt_dfs) {
  ...
  df_long <- tidyr::pivot_longer(
    ...
    values_to = "PHS"            # Coherence values
  )
  ...
  df_long <- df_long[, c("ID", "Hz", "TIME", "PHS")]
...
output_dir_A <- file.path("..\\NONLINEAR_PATTERN_MODELING\\PHASE_ANGLE\\", "FRQ_OPT_VERSION_A_PHS_LONG")
output_dir_B <- file.path("..\\NONLINEAR_PATTERN_MODELING\\PHASE_ANGLE\\", "FRQ_OPT_VERSION_B_PHS_LONG")
...
for (df_name in df_names_A) {
  ...
  # Construct file name
  file_name <- paste0("FRQ_OPT_A_LONG_", id, "_PHS.csv")
...
for (df_name in df_names_B) {
  ...
  # Construct file name
  file_name <- paste0("FRQ_OPT_B_LONG_", id, "_PHS.csv")
...
for (df_name in ds_dfs) {
  ...
  # Reorder and select final columns: ID, Period, TIME, PHS
  df_long <- df_long[, c("ID", "Hz", "TIME", "PHS")]
...
# DEFINE TARGET DIRECTORY
output_dir_HF <- file.path("..\\NONLINEAR_PATTERN_MODELING\\PHASE_ANGLE\\", "HI_FRQ_PHS_LONG")
output_dir_LF <- file.path("..\\NONLINEAR_PATTERN_MODELING\\PHASE_ANGLE\\", "LO_FRQ_PHS_LONG")
...
for (df_name in df_names_HF) {
  ...
  # Construct file name
  file_name <- paste0("HF_LONG_", id, "_PHS.csv")
...
for (df_name in df_names_LF) {
  ...
  # Construct file name
  file_name <- paste0("LF_LONG_", id, "_PHS.csv")
