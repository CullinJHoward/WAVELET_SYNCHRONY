...
################################################################################
############## TRIM DATA TO OPTIMIZED COHERENCE BINS PREVIOUSLY IDENTIFIED
...
for (df_name in coherence_dfs) {
  ...
  oc_ub <- summary_row$BIN_UB
  oc_lb <- summary_row$BIN_LB
...
MEAN_BOPT_COH_SUMMARY <- data.frame(ID = character(), W_BOPT_MEAN_COH = numeric(), stringsAsFactors = FALSE)
...
for (df_name in ls(pattern = "^OPT_")) {
  ...
  # Store results
  MEAN_BOPT_COH_SUMMARY <- rbind(MEAN_BOPT_COH_SUMMARY, data.frame(ID = ID, W_BOPT_MEAN_COH = final_mean))
}
...
OUTPUT_TINV_BOPT_BIN_MEANS <- file.path(work_dir, "TIME_INV_OPT_BIN_COH_SCORES.csv")

# Save the data frame as a CSV file
write.csv(MEAN_BOPT_COH_SUMMARY, file = OUTPUT_TINV_BOPT_BIN_MEANS, row.names = FALSE)
...
for (df_name in ls(pattern = "^TRU_RED_OPT_")) {
  ...
  df_long <- data.frame(
    ...
    BOPT_COH_RAW = as.vector(unlist(df))  # Flatten the dataframe to a single column
  )
  
  # Compute the mean and standard deviation of BOPT_COH_RAW, ignoring NAs and NaNs
  mean_coh_raw <- mean(df_long$BOPT_COH_RAW, na.rm = TRUE)
  sd_coh_raw <- sd(df_long$BOPT_COH_RAW, na.rm = TRUE)
  
  # Create OPT_COH_CEN by centering BOPT_COH_RAW and replacing NAs/NaNs with 0
  df_long$BOPT_COH_CEN <- ifelse(is.na(df_long$BOPT_COH_RAW) | is.nan(df_long$BOPT_COH_RAW), 
                               0,  # Replace NAs and NaNs with 0 instead of the mean
                               df_long$BOPT_COH_RAW - mean_coh_raw)
  
  # Create OPT_COH_STD by standardizing BOPT_COH_RAW and replacing NAs/NaNs with 0 (mean)
  df_long$BOPT_COH_STD <- ifelse(is.na(df_long$BOPT_COH_RAW) | is.nan(df_long$BOPT_COH_RAW) | sd_coh_raw == 0, 
                               0,  # Replace NAs and NaNs in OPT_COH_STD with 0 instead of the mean
                               (df_long$BOPT_COH_RAW - mean_coh_raw) / sd_coh_raw)
...
hist(OPT_COH_JOINT$BOPT_COH_RAW)
...
# Compute the BOPT_MEAN_COH for each ID (mean of COH_RAW) and repeat for all rows within the same ID
OPT_COH_JOINT <- OPT_COH_JOINT %>%
  group_by(ID) %>%
  mutate(BOPT_MEAN_COH = mean(BOPT_COH_RAW, na.rm = TRUE)) %>%
  ungroup()  # Ungroup after mutation to avoid future issues with grouped data

## GRAND MEAN CENTERED RAW MEANS 

# Compute the grand mean of BOPT_MEAN_COH
grand_mean_coh <- mean(OPT_COH_JOINT$BOPT_MEAN_COH, na.rm = TRUE)

# Create the MEAN_OPT_COH_CEN variable by centering BOPT_MEAN_COH with the grand mean
OPT_COH_JOINT <- OPT_COH_JOINT %>%
  mutate(MEAN_BOPT_COH_CEN = BOPT_MEAN_COH - grand_mean_coh)
...
# Define the file path where you want to save the CSV
OUTPUT_TVAR_OPT_BIN_COH <- file.path(work_dir, "TIME_VAR_BOPT_BIN_COH_SCORES.csv")

# Save the data frame as a CSV file
write.csv(OPT_COH_JOINT, file = OUTPUT_TVAR_OPT_BIN_COH, row.names = FALSE)
